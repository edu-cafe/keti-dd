...
_____ func_workqueue( _________ data)
{
	int i;
...
static ssize_t led_write (struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	while(!flag_timer0Int) {
		...
		ret = wait_event_interruptible(wq_timer, flag_timer0Int);
...
static int __init led_init(void){
	...
	//result = request_irq(IRQ_TIMER0_VIC, timer0_int, IRQF_TIMER|IRQF_DISABLED, "MyTimer", NULL);
	...
	//include/asm-generic/errno-base.h     22:EINVAL
